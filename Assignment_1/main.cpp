...
#include <queue>
int menu();
void driver(Yardmaster* yardMaster, Scrapyard* scrapYard, std::list<Vehicle*>& vehicleCollection, int numDays, std::ofstream& myFile, std::default_random_engine* gptr, RepairBay* repairBays);
...
void fillRepairBay(std::queue<Vehicle*>& vehicleQueue, RepairBay* repairBay);
...
int main(int argc,char* argv[]){
    ...
    //Repair Bay 
    RepairBay* repairBays = new RepairBay();
...
void fillRepairBay(std::queue<Vehicle*>& vehicleQueue, RepairBay* repairBay){
    ...
    if(!vehicleQueue.empty()){
        for(int i = 0; i<vehicleQueue.size(); i++){
            ...
            repairBay->addToRepair(vPtr);
            full = repairBay->bayFull();
